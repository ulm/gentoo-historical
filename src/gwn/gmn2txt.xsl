...
<xsl:variable name="all-links">
  <links>
    <xsl:for-each select="//uri[@link]|//mail">
     <xsl:choose>
      <xsl:when test="name(.)='uri' and not(starts-with(@link, '#')) and not(starts-with(@link, '?')) and not(@link=normalize-space(.))">
        <uri>
      ...
      <xsl:when test="name(.)='mail' and not(name(..)='author')">
       <xsl:variable name="mail">
        <xsl:call-template name="smart-mail">
         <xsl:with-param name="mail" select="."/>
        </xsl:call-template>
       </xsl:variable>
       <xsl:if test="(string-length(exslt:node-set($mail)/mail/@link)>0) and not(exslt:node-set($mail)/mail/@link = exslt:node-set($mail)/mail/text())">
        <uri>
         <xsl:attribute name="id"><xsl:value-of select="generate-id(.)"/></xsl:attribute>
         <xsl:value-of select="exslt:node-set($mail)/mail/@link"/>
        </uri>
       </xsl:if>
      </xsl:when>
     </xsl:choose>
...
<xsl:template match="guide">
  <xsl:variable name="header">
    ...
    <xsl:text>&#xA;http://www.gentoo.org/news/</xsl:text><xsl:value-of select="$lang"/><xsl:text>/gmn/&#xA;</xsl:text>
    <xsl:call-template name="wrap-text">
  ...
  <xsl:for-each select="author">
    ...
    <xsl:if test="string-length(exslt:node-set($mail)/mail/@link)>0">
     <xsl:text> &lt;</xsl:text>
      <xsl:value-of select="exslt:node-set($mail)/mail/@link"/>
     <xsl:text>&gt;</xsl:text>
    </xsl:if>
