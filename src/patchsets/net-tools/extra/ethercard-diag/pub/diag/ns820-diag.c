...
"Usage: ns820-diag [-aEefFGhmqrRtTvVwW] [-p <IOport>] [-[AF] <media>]\n"
...
extern int  ns820_diag(int vend_id, int dev_id, long ioaddr, int part_idx);
...
struct pcidev_entry {
	...
	{"National Semiconductor DP83820 series", 0,
	 0x100B, 0x0022, 0xffff, 0, 256, ns820_diag},
	{ 0, 0, 0, 0},
};
...
/* The chip-specific section for the dp83820 series. */
...
enum register_offsets {
	...
	TxRingPtr=0x20, TxRingPtrHi=0x24, TxConfig=0x28, GeneralIO=0x2C,
...
static const char *intr_names[] ={
	...
	"Rx status overrun",  "PCI target abort",  "PCI master abort",
	"PCI system error", "PCI parity error", "Receiver reset done",
...
static void natsemi820_eeprom(unsigned short *ee_data);
...
int ns820_diag(int vendor_id, int device_id, long ioaddr, int part_idx)
{
	...
	printf(" Natsemi 83820 series with station address ");
	for (i = 0; i < 6; i+=2) {
	...
	if (verbose > 1 || show_regs) {
		...
		unsigned int tx_config = inl(ioaddr + TxConfig);
		...
		printf("  Tx threshold %d bytes, FIFO fill %d bytes.\n",
			   (tx_config & 0xff) << 5, (tx_config & 0xff00) >> 3); 
	}
	...
	if (do_test && chip_active && !opt_f)
		...
		int result;
		outl(0x0400, ioaddr + TestControl);
		for (i = 0; i < 10000; i++) { 		/* Typical 1050 iterations. */
			if (inl(ioaddr + TestControl) & 0x0200)
				break;
		}
		...
		printf(" Internal SRAM test result is %4.4x: %s.\n",
			   result, result & 0x01B8 ? "failed" : "passed");
		if (result & 0x01B8) {
			if (result & 0x0100)
				fprintf(stderr, " *** Rx status FIFO test failed!\n");
			if (result & 0x0080)
				fprintf(stderr, " *** Rx data FIFO test failed!\n");
			if (result & 0x20)
				fprintf(stderr, " *** Tx data FIFO test failed!\n");
			if (result & 0x0010)
				fprintf(stderr, " *** Rx multicast filter test failed!\n");
			if (result & 0x0008)
				fprintf(stderr, " *** Rx match filter test failed!\n");
			if ( ! opt_f)
				return 2;
		}
		outl(0x02, ioaddr + TestControl);
		for (i = 0; i < 10000; i++) { 		/* Typical 3350 iterations. */
	...
	if (set_hwaddr) {
		u16 backup_ee_contents[] = {
			0x4900, 0x1186, 0x340b, 0x0200, 0x0000, 0x00b4, 0x0000, 0x0000,
			0x0000, 0x24a7, 0x37ba, 0x5000, };
		if (emergency_rewrite)
			memcpy(new_ee_contents, backup_ee_contents,
				   sizeof backup_ee_contents);
		/* NatSemi corrected the endian mix-up and dropped bits of the
		   earlier series. */
		new_ee_contents[12] = new_hwaddr[0] + (new_hwaddr[1]<<8);
		new_ee_contents[11] = new_hwaddr[2] + (new_hwaddr[3]<<8);
		new_ee_contents[10] = new_hwaddr[4] + (new_hwaddr[5]<<8);
		eeprom_change++;
	}
	if (eeprom_change) {
		int sum = 0x55;
		...
		for (i = 0; i < 13; i++)
			sum += new_ee_contents[i] + (new_ee_contents[i]>>8);
		new_ee_contents[13] = ((-sum & 0xff) << 8) | 0x0055;
		for (i = 0; i < 16; i++)
	...
	if (show_eeprom)
		natsemi820_eeprom(eeprom_contents);
	...
	printf(" General purpose I/O register is %8.8x.\n",
		   inl(ioaddr + GeneralIO));
#ifdef LIBFLASH
	{
...
static void natsemi820_eeprom(unsigned short *ee_data)
{
	...
	printf("Decoded EEPROM contents:\n"
		   ...
		   ee_data[1], ee_data[0], ee_data[2] & 0xff, ee_data[2] >> 8);
	printf("  Wake-On-LAN password "
		   "%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x.\n",
		   ee_data[9] & 0xff, ee_data[9]>>8,
		   ee_data[8] & 0xff, ee_data[8]>>8,
		   ee_data[7] & 0xff, ee_data[7]>>8);
	printf("  Ethernet MAC Station Address "
		   "%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x.\n",
		   ee_data[12] & 0xff, ee_data[12]>>8,
		   ee_data[11] & 0xff, ee_data[11]>>8,
		   ee_data[10] & 0xff, ee_data[10]>>8);
	printf("  General Purpose I/O register %4.4x.\n", ee_data[4]);
	for (i = 0; i < 13; i++)
		sum += ee_data[i] + (ee_data[i]>>8);
	sum = -sum & 0xff;
	printf("  EEPROM active region checksum read as %4.4x, vs %2.2x55 "
		   "calculated value.\n",
		   ee_data[13], sum);
	return;
}
...
/*  Unlike the 83815 series, the 820 series has only a serial interface to
	...
	The maxium data clock rate is 2.5 Mhz, which can be exceeded unless
	we force out buffered PCI writes.
*/
#define mdio_out(value, mdio_addr) outl(value, mdio_addr)
#define mdio_in(mdio_addr) inl(mdio_addr)
#define mdio_delay(mdio_addr) mdio_in(mdio_addr)

#define MDIO_IO_OFFSET EECtrl

#define MDIO_SHIFT_CLK	0x40
#define MDIO_DATA_BIT	0x10
/* Derived values. */
#define MDIO_WRITE0 0x20
#define MDIO_WRITE1 0x30
...
/*
 * Local variables:
 *  compile-command: "cc -O -Wall -Wstrict-prototypes -o ns820-diag ns820-diag.c `[ -f libmii.c ] && echo -DLIBMII libmii.c` `[ -f libmii.c ] && echo -DLIBFLASH libflash.c`"
 *  simple-compile-command: "cc -O -Wall -o ns820-diag ns820-diag.c"
 *  tab-width: 4
 *  c-indent-level: 4
 *  c-basic-offset: 4
 * End:
