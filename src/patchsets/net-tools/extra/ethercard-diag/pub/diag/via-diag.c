/* via-diag.c: Linux Diagnostic and setup code for the VIA Rhine chips.
   ...
   based on the following chips:
	VIA Rhine vt86c100, vt3043, vt6102, and vt6105 Ethernet controllers.
...
"via-diag.c:v2.10 2/28/2005 Donald Becker (becker@scyld.com)\n"
" http://www.scyld.com/diag/index.html\n";
static char *usage_msg =
"Usage: via-diag [-aDEefFGhmqrRtvVwW] [-p <IOport>] [-[AF] <media>]\n"
...
struct pcidev_entry {
	...
	{"VIA VT86C100A Rhine",
	 ...
	 0x1106, 0x6100, 0xffff, 0, 128, rhine_diag},
	{"VIA VT3043 Rhine",
	 " Ethernet controller: VIA Technologies Unknown device",
	 0x1106, 0x3043, 0xffff, 0, 128, rhine_diag},
	{"VIA VT6102 Rhine-II (type 3065)", 0,
	 0x1106, 0x3065, 0xffff, CanFlash, 256, rhine_diag},
	{"VIA VT6105 Rhine-III (type 3053 prototype)", 0,
	 0x1106, 0x3053, 0xffff, CanFlash, 256, rhine_diag},
	{"VIA VT6105 Rhine-III (type 3106)", 0,
	 0x1106, 0x3106, 0xffff, CanFlash, 256, rhine_diag},
	{ 0, 0, 0, 0},
};
...
static void rhine_eeprom(unsigned short *ee_data);
...
enum register_offsets {
	...
	StickyHW=0x83,
	FlashAddr=0x8C, FlashWrite=0x8f, FlashStatus=0x90, FlashRead=0x91,
	WOLcrClr=0xA4, WOLcgClr=0xA4, PwrcsrClr=0xAC,
};
...
static int rhine_flash_in(long ioaddr, int offset) {
	outw(offset, ioaddr + FlashAddr);
	outb(1, ioaddr + FlashStatus);
	return inb(ioaddr + FlashRead);
}
static void rhine_flash_out(long ioaddr, int offset, int val) {
	outw(offset, ioaddr + FlashAddr);
	outb(val, ioaddr + FlashWrite);
	outb(2, ioaddr + FlashStatus);
	return;
}
#endif
...
static struct config_name rcvr_mode[] = {
	{0x08, 0x1f, "Normal unicast"},
	{0x0C, 0x1f, "Normal unicast and hashed multicast"},
	{0x1C, 0x08, "Promiscuous"},
	{0x00, 0x00, "Unknown/invalid"},
};
static struct config_name txmr_mode[] = {
	{0x00, 0xE6, "Normal transmit, 128 byte threshold"},
	{0x20, 0xE6, "Normal transmit, 256 byte threshold"},
	{0x40, 0xE6, "Normal transmit, Tx threshold increased to 512 bytes"},
	{0x60, 0xE6, "Normal transmit, Tx threshold increased to 1024 bytes"},
	{0x80, 0x86, "Normal transmit, transmitter set to store-and-forward"},
	{0x02, 0x06, "Transmitter set to INTERNAL LOOPBACK!"},
	{0x04, 0x06, "Transmitter set to ENDEC/MII LOOPBACK!"},
	{0x06, 0x06, "Transmitter set to EXTERNAL LOOPBACK!"},
	{0x00, 0x00, "Unknown/invalid"},
};
...
int rhine_diag(int vendor_id, int device_id, long ioaddr, int part_idx)
{
	...
	int tx_mode = inb(ioaddr + TxConfig);
	int chip_cmd = inw(ioaddr + ChipCmd);
	int ee_addr_len = 6, eeprom_size = 32;
	int i;

	if (! opt_f  &&  inl(ioaddr) == 0xffffffff) {
	...
	printf(" Tx %sabled, Rx %sabled, %s-duplex (0x%4.4x).\n",
		   chip_cmd & 0x10 ? "en" : "dis",
		   chip_cmd & 0x08 ? "en" : "dis",
		   chip_cmd & 0x0400 ? "full" : "half", chip_cmd);
	...
	for (i = 0; txmr_mode[i].mask; i++)
		if ((tx_mode & txmr_mode[i].mask) == txmr_mode[i].val) break;
	printf("  Transmit mode is 0x%2.2x: %s.\n", tx_mode, txmr_mode[i].name);
	...
	if (verbose || show_regs) {
		...
		if (opt_a) {
			/* Reading some registers disrupts chip operation. */
	...
	if (show_eeprom || set_hwaddr || new_default_media >= 0) {
		/* The Rhine has the questionable ability to 'lock' EEPROM access.
		   EEPROM sizing cannot be done unless the EEPROM is unlocked. */
...
#if defined(HAS_EEPROM_RECOVERY)
	...
	if (show_eeprom > 1) {
		printf("EEPROM contents%s:",
			   inb(ioaddr + EECtrl) & 0x80 ?
			   " (Assumed from chip registers)" : "");
	...
	if ((opt_flash_loadfile  || opt_flash_dumpfile  ||  opt_flash_show)
		&&  (CanFlash & flags) == 0)
		printf("This chip type does not support Flash access or "
			   "programming.\n");
#ifndef LIBFLASH
	if (opt_flash_loadfile  || opt_flash_dumpfile  ||  opt_flash_show)
...
enum EEPROM_Ctrl_Bits {
	EE_ShiftClk=0x04, EE_ChipSelect=0x08, EE_DataOut=0x02, EE_DataIn=0x01,
	EE_Write0=0x08, EE_Write1=0x0A,
};
...
#define decode_ee_fmt \
"Decoded EEPROM contents:\n"\
"   Ethernet MAC Station Address %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x.\n"\
"   PHY at MII #%d.\n"\
...
"  Checksum computed as %2.2x, read as %2.2x.\n"
...
static void rhine_eeprom(unsigned short *ee_data)
{
	...
	printf(decode_ee_fmt,
		   ee_data[0] & 0xff, ee_data[0]>>8,
		   ee_data[1] & 0xff, ee_data[1]>>8,
		   ee_data[2] & 0xff, ee_data[2]>>8,
		   ee_data[3] & 0xff,						/* PHY */
		   ee_data[5], ee_data[4], 					/* Subsystem */
		   ee_data[7], ee_data[6], 					/* Subsystem */
		   ee_data[11] & 0xff, ee_data[11]>>8,		/* PCI bus config */
		   checksum & 0xff, ee_data[15] >> 8);
	return;
}
