...
# This section will ask whether to sync the tree, whether to use a snapshot, etc.
	...
	code, portage_tree_sync = d.menu("How do you want to sync the portage tree?  (emerge sync is temporarily disabled, use webrsync instead)", choices=dmenu_list_to_choices(menulist))
	if code != DLG_OK: return
	portage_tree_sync = menulist[int(portage_tree_sync)-1]
	#FIX ME when python 2.4 comes out.
	if portage_tree_sync == "Normal 'emerge sync' (temporarily disabled)": install_profile.set_portage_tree_sync_type(None, "webrsync", None)
