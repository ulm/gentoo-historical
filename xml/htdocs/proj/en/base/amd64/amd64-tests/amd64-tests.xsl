...
<guide link="amd64-tests.xml">

<title>
Gentoo/AMD64 Project Test Requests
</title>
...
<xsl:for-each select="test">
  ...
  <ti>
    <xsl:choose>
     <xsl:when test="@bug != 'n/a'"> <!-- Test on? -->
        <a href="http://bugs.gentoo.org/show_bug.cgi?id={@bug}"><xsl:value-of select="@bug"/></a>
     </xsl:when>
    </xsl:choose>
  </ti>
  <ti>
