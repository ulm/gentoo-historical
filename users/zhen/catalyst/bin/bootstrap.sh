...
# $Header: /var/cvsroot/gentoo/users/zhen/catalyst/bin/bootstrap.sh,v 1.1 2003/09/09 06:24:51 zhen Exp $
...
# <zhen@gentoo.org> hardnened stages, etc
if [ -n ${BUILD_HARDENED} ]
	myHARDENED_GCC=`cat ${MYPROFILEDIR}/packages|grep -v '^#'|grep sys-devel/hardened-gcc |sed 's:^\*::'`
fi
...
# <zhen@gentoo.org> emerge twice to keep us hardened after gcc builds
if [ -n ${BUILD_HARDENED} ]
then
